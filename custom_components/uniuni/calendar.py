"""Calendar platform for the UniUni parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the UniUni deliveries calendar from a config entry."""
    async_add_entities([UniUniDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class UniUniDeliveriesCalendar(CoordinatorEntity[UniUniCoordinator], CalendarEntity):
    """A read-only calendar of expected UniUni deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "UniUni parcel")
            description_parts = [
