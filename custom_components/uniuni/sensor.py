"""Sensor platform for the UniUni parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: UniUniConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up UniUni sensor entities from a config entry."""
    ...
    for parcel in coordinator.data or []:
        entities.append(
            UniUniParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(UniUniNextDeliverySensor(coordinator, entry))
    entities.append(UniUniDeliveredParcelsSensor(coordinator, entry))
    entities.append(UniUniLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class UniUniIncomingParcelsSensor(
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                UniUniParcelSensor(self.coordinator, self._entry, barcode)
...
class UniUniParcelSensor(CoordinatorEntity[UniUniCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked UniUni parcel."""
    ...
    def __init__(
        self, coordinator: UniUniCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class UniUniNextDeliverySensor(
    CoordinatorEntity[UniUniCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: UniUniCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class UniUniDeliveredParcelsSensor(
    CoordinatorEntity[UniUniCoordinator], SensorEntity
):
    """Recently delivered tracked UniUni parcels."""
...
class UniUniLastUpdateSensor(
    CoordinatorEntity[UniUniCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when UniUni was last polled successfully."""
