"""Services for the UniUni parcel tracker integration.

`uniuni.track_parcel` / `uniuni.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single UniUni hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("UniUni is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the UniUni services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid UniUni tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the UniUni services (single-entry integration, so on unload)."""
